...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = initial match{
    ...
    case(block,move_xs)#::rest=>{
      val next = newNeighborsOnly(neighborsWithHistory(block,move_xs),explored+block)
      next ++ from(rest ++ next, explored+block)
    }
  }
  ...
  /**
    ...
    */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {///starting block
  ...
  lazy val solution: List[Move] = pathsToGoal match{
    ...
    case(block,move_xs)#::tail=>move_xs.reverse
